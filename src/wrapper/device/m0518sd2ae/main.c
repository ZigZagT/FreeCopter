...
void I2C_Slave_Go(I2C_T* port, uint32_t status) {
    ...
    switch (status) {              
        case I2C_STATUS_SLAVE_RESTART_STOP:
        ...
        I2C_SET_CONTROL_REG(port, I2C_I2CON_SI_AA);
        if (status == I2C_STATUS_SLAVE_SEND_DATA_NACK) {
            printf("I2C_STATUS_SLAVE_SEND_DATA_NACK\n");
        }
        break;
        ...
        // if (status == I2C_STATUS_SLAVE_SEND_ADDR_ACK) {
        //     printf("I2C_STATUS_SLAVE_SEND_ADDR_ACK\n");
        // }
        if (status == I2C_STATUS_SLAVE_SEND_DATA_ACK) {
            printf("I2C_STATUS_SLAVE_SEND_DATA_ACK\n");
        }
        //printf("I2C status 0x%X\n", status);
        if(I2C_Send_Byte(&I2C_Send_Control, port, NULL) == I2C_NOTICE_LAST_DATA_SENT) {
        ...
        I2C_Recv_Byte(&I2C_Recv_Control, port, NULL);
        I2C_SET_CONTROL_REG(port, I2C_I2CON_SI_AA);
        break;
